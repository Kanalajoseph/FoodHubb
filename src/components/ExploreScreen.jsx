  ...
  return (
    <div className="min-h-screen pb-24 chitenge-bg">
      ...
      <div className="px-5 pt-14 pb-4">
        <h1 className="font-display text-2xl font-bold text-earth-800 animate-fade-in-up">
          Explore Zambian
          <span className="text-terracotta-500"> Cuisine</span>
        </h1>
        <p className="text-earth-400 text-sm mt-1 animate-fade-in-up stagger-1">
      ...
      <div className="px-5 mb-4 animate-fade-in-up stagger-2">
        <div className="flex items-center gap-3 bg-white rounded-2xl px-4 py-3 shadow-sm border border-earth-100">
          ...
          <span className="text-earth-300 text-sm">Search dishes, ingredients...</span>
        </div>
      </div>
      ...
      <div className="px-5 mb-3">
        <p className="text-earth-400 text-xs font-medium">
        ...
        <div className="grid grid-cols-2 gap-3">
          {filteredDishes.map((dish, i) => (
            ...
            >
              ...
              <div className={`relative h-28 bg-gradient-to-br ${dish.gradient} flex items-center justify-center overflow-hidden`}>
                {/* Pattern overlay */}
                <div className="absolute inset-0 opacity-10 bg-[repeating-linear-gradient(60deg,white_0px,white_1px,transparent_1px,transparent_6px)]" />
                <span className="text-5xl relative z-10 group-hover:scale-110 transition-transform duration-300">
                  {dish.emoji}
                </span>

                {/* Rating badge */}
                <div className="absolute top-2 right-2 bg-black/30 backdrop-blur-sm text-white text-[10px] font-semibold px-2 py-0.5 rounded-full flex items-center gap-0.5">
              ...
              {/* Card body */}
              <div className="p-3">
                <h3 className="font-semibold text-sm text-earth-800 group-hover:text-terracotta-600 transition-colors">
                  {dish.name}
                </h3>
                <p className="text-[11px] text-earth-400 mt-0.5 line-clamp-1">
                  {dish.subtitle}
                </p>
                ...
                <div className="flex flex-wrap gap-1 mt-2">
                  {dish.tags.slice(0, 2).map((tag) => (
                    <span
                      key={tag}
                      className="text-[9px] font-medium bg-terracotta-50 text-terracotta-600 px-2 py-0.5 rounded-full border border-terracotta-100"
                    >
                      {tag}
                    </span>
                  ))}
